...
public class WorldTicker implements ITickHandler {
    ...
    public static Map<Integer, LinkedBlockingQueue<VirtualBreaker>> breakList = new HashMap();
    ...
    public String getLabel() {
        return "ElementalWorld";
    }

    private void breakTicks(WorldServer world) {
        int dim = world.provider.dimensionId;
        LinkedBlockingQueue queue = (LinkedBlockingQueue) breakList.get(Integer.valueOf(dim));
    ...
    public static void addSwapper(World world, int x, int y, int z, int id, int meta, int idT, int metaT, int life, EntityPlayer player) {
        ...
        LinkedBlockingQueue queue = (LinkedBlockingQueue) breakList.get(Integer.valueOf(dim));
        if (queue == null) {
            breakList.put(Integer.valueOf(dim), new LinkedBlockingQueue());
            queue = (LinkedBlockingQueue) breakList.get(Integer.valueOf(dim));
        }
