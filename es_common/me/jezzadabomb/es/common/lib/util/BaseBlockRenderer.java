package me.jezzadabomb.es.common.lib.util;
...
public abstract class BaseBlockRenderer implements ISimpleBlockRenderingHandler {
    ...
    protected static void renderAsItem(Block block, RenderBlocks renderer, Icon[] tmap) {
        ...
        renderer.renderFaceXPos(block, 0, 0, 0, tmap[2]);
        tessellator.draw();

        // X-
        tessellator.startDrawingQuads();
        tessellator.setNormal(0.0F, 0.0F, 1.0F);
        renderer.renderFaceXNeg(block, 0, 0, 0, tmap[3]);
        tessellator.draw();

        // Z-
        tessellator.startDrawingQuads();
        tessellator.setNormal(-1.0F, 0.0F, 0.0F);
        renderer.renderFaceZNeg(block, 0, 0, 0, tmap[4]);
        tessellator.draw();

        // Z+
        tessellator.startDrawingQuads();
        tessellator.setNormal(1.0F, 0.0F, 0.0F);
        renderer.renderFaceZPos(block, 0, 0, 0, tmap[5]);
        tessellator.draw();
